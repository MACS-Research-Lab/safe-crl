...
def main(args, cfg_env=None):
    ...
    for epoch in range(epochs):
        ...
        # comnpute advantage
        advantage = data["adv_r"] # cost critic is not used
        ...
        for _ in range(config["learning_iters"]):
            for (
                ...
                cost_weight = 5
                episode_cost = np.mean(cost_deque)

                total_loss = loss_pi + 2*loss_r + loss_c + ewc_loss * lambda_ewc - cost_weight * episode_cost\
                    if config.get("use_value_coefficient", False) \
                    else loss_pi + loss_r + loss_c + ewc_loss * lambda_ewc - cost_weight * episode_cost
                ...
                logger.store(
                    **{
                        ...
                        "Loss/Loss_safety": cost_weight * episode_cost
                    }
                )
        ...
        if not logger.logged:
            ...
            logger.log_tabular("Loss/Loss_safety")
