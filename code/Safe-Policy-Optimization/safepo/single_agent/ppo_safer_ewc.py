...
# Modified to include EWC and penalized weight update by cost
...
def compute_fisher_info(data, policy):
    fisher_info = {}
    for obs, act, cost in zip(data['obs'], data['act'], data['cost']):
        policy.zero_grad()
        ...
        for name, param in policy.actor.named_parameters():
            ...
            if name not in fisher_info:
                ...
                fisher_info[name] = weighted_grad_sq.clone()
                ...
                fisher_info[name] += weighted_grad_sq
