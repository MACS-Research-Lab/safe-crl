...
def get_hyperparameters(config, task):
    if task == 'SafetyHalfCheetahVelocity-v4':
        ...
        env_name = 'cw'
    elif task == 'SafetyAntVelocity':
        env_name = 'ant'
    else:
        return config
    
    db_path = os.path.abspath(f"./hyperparams/cpo_{env_name}.db")
    study = optuna.load_study(study_name=f'cpo_{env_name}', storage=f'sqlite:///{db_path}')
